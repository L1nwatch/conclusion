...
Conclusion 写入时会校验 `decisionAnalysis` 引用的模型 ID、版本和统一的 `analysis` 键。每个模型的业务定义只有名称和简短解释；FengDock MCP 不重复维护模型常量，直接复用 Conclusion 数据库函数。`update_decision_model` 创建下一不可变版本，不覆盖被历史 Conclusion 引用的模型。
